...
群聊参与：世界状态 · 今日日报 · 事件记录 · 据点车间与图鉴 · 名片与排行榜

[查看实际界面](#actual-ui) · [快速开始](#quick-start) · [完整指令](docs/commands.md) · [问题反馈](https://github.com/SolitudeRA/astrbot_plugin_palworld/issues)

状态采集只读 · 受控写默认关闭 · 服务器管理命令仅授权管理员<br>
玩家观测库不存储 IP · 群聊不公开精确位置 —— 详见[安全与边界](#security-boundaries)
...
完成首次设置并授权群聊后，成员可以查询世界状态、服务器规则、在线玩家、今日日报和事件记录，也能围观据点车间现场、翻看服务器图鉴；玩家档案与排行榜是否开放，由服主自行决定。这些内容按需查询、不会主动刷屏，却能让世界进度、在线纪录与玩家成长自然成为群里的共同话题。`/pal help` 会根据当前模式、功能开关和使用者身份，只展示实际可用的命令。
...
帕鲁世界的日常也能成为群里的谈资：据点详情会汇总此刻可见帕鲁的行为分布与摸鱼率，图鉴则记录服务器上曾被观测到的物种；开启玩家档案后，成员还能亮出个人名片，`card`／`卡` 可生成图片版，浅色或暗色主题由服主设置。
...
此刻可见 12 只 · 活跃 9 · 平均 Lv23.4
状态 🟢 健康 · 平均HP 88%
🚬 摸鱼率 17%

行为分布
· ⛏5 工作中 · 🚶2 移动 · 🚬2 摸鱼 · 🛌2 睡觉
...
今日在线 2时14分 · 累计 1天22时
公会「先锋」
随身 空涡龙（龙）Lv50 · HP 92% · 随行
```

<p align="center">
  <img src="./docs/images/me-card-light.png" alt="个人名片图片版浅色主题，演示数据" width="49%">
  <img src="./docs/images/me-card-dark.png" alt="个人名片图片版暗色主题，演示数据" width="49%">
</p>
...
| 玩家档案 | **关** | 玩家查询、绑定、排行榜和个人名片由服主按需开启；名片可出图片版 |
...
打开实际生效的 `Pal/Saved/Config/WindowsServer/PalWorldSettings.ini` 或 `LinuxServer/PalWorldSettings.ini`，设置高强度的 `AdminPassword`、`RESTAPIEnabled=True` 和 `RESTAPIPort=8212`（可按部署修改端口），保存后重启 PalServer。`DefaultPalWorldSettings.ini` 只是模板，修改它不会生效。
...
| `/pal guild base <名称\|#序号>` | 开 | 据点车间现场（行为分布、摸鱼率与氛围徽章） |
| `/pal dex` | 开 | 服务器图鉴（已观测物种，按元素分桶点亮） |
...
多服模式使用 `/pal link list/add/remove` 管理本群服务器授权；查询命令可在末尾添加 `@<服务器名>`，例如 `/pal world status @alpha`。管控命令使用本群当前活动服务器，切换目标后再执行。完整参数、关闭行为和权限矩阵见[完整指令文档](docs/commands.md)。
...
更多服务器连接、群聊授权、轮询、凭证、隐私和模式转换说明，见[配置文档](docs/configuration.md)。
...
- [配置项详解](docs/configuration.md) —— 服务器、路由、权限、轮询、隐私、凭证与设置页
- [完整指令文档](docs/commands.md) —— 命令树、参数、默认状态、路由与降级行为
- [贡献指南](CONTRIBUTING.md) —— 开发环境、测试、前端构建与提交约定
