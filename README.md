...
- **服务器管控(受控写)** —— 广播、存档、踢人、封禁、倒计时关服等 7 条写命令 + 二次确认;**默认全部关闭、仅授权管理员可用、全程落库审计**(`/pal announce`、`/pal kick`、`/pal shutdown` 等)
...
- **隐私优先** —— 观测只读、不存 IP、玩家标识 HMAC 哈希落库、坐标量化为粗网格;写操作全程审计,目标 userid 仅以哈希落库
