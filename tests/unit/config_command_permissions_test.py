...
def test_guild_enabled_on_row_takes_effect():
    # 解禁后 guild 组/叶行 enabled=on 正常生效（不再被 force-off 拦截，不入 invalid）。
    cfg = _cfg({"command_permissions": [
        _row("guild list", enabled="on"),
        _row("guild", enabled="on"),
    ]})
    assert cfg.permissions.invalid_command_keys == []
    ov = cfg.permissions.command_overrides
    assert ee(ov, "guild list") is True        # 叶子 on 生效
    assert ee(ov, "guild info") is True         # 组 on 传导
