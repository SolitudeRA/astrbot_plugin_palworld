...
插件目录本身不在 sys.path 上——顶级绝对导入 palchronicle 会直接
...
def test_main_importable_via_namespaced_package_without_repo_root_on_sys_path():
    ...
    saved_modules = {
        ...
        if k == "palchronicle" or k.startswith("palchronicle.")
    }
    try:
        # 模拟 AstrBot：仓库根不在 sys.path，palchronicle 未被预先导入
        sys.path = [
            p for p in sys.path
        ...
        assert hasattr(mod, "PalChronicle")
    finally:
        sys.path[:] = saved_path
        for k in list(sys.modules):
