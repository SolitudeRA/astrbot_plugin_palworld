...
<template>
  ...
  <div v-if="mode === 'view'" class="card">
    <div class="card-head">
      ...
      <span class="nm mono">{{ (modelValue.id as string) || '（未填）' }}</span>
    ...
    <div class="cbody">
      <div class="crow"><span class="ck">标识</span><span class="cv mono">{{ (modelValue.id as string) || '（未填）' }}</span></div>
      ...
      <div class="crow">
        ...
        <span class="cv">
          <input class="pw-input mono" type="text"
            ...
            placeholder="如 aiocqhttp:12345"
            :value="(draft.id as string) ?? ''"
            @input="setDraft('id', ($event.target as HTMLInputElement).value)" />
        </span>
      </div>
      <div class="crow">
