...
- **等级榜**：对**已观测玩家**按 `latest_level` 降序 → 过滤 `excluded_keys` → 取前 N，经新增 `Repository.list_players_by_level(world_id) -> list[PlayerIdentity]`（`SELECT ... FROM players WHERE world_id=? AND latest_level IS NOT NULL ORDER BY latest_level DESC`；`players` 表当前无任何列表查询方法，须新写）。**同名去重**：同一真人可能有 HIGH/LOW 两个 `player_key` 行（三级身份回退），榜单按 `latest_name` 去重、保留 `last_seen_at` 最新的一行，避免同名双行。行：`· {名} Lv{n}`。
  - **暴露面登记（见 §5.4）**：等级榜使离线玩家的等级可查（此前仅在线玩家经 `list_open_sessions` 可见），属 players 组相对 core 的**新增个体暴露面**，随组默认关而收敛；**strict 模式下等级榜仅取当前在线玩家**（对齐在线维度、不复活休眠数据）。
- **strict 模式**：只出**等级榜**（且仅在线，见上），时长榜停用（时长≈作息）；`/pal rank time` 在 strict 回 `L("rank_time_strict")`（「时长榜在 strict 隐私模式下停用」）。
...
- **等级榜暴露**：登记为 players 组相对 core 的新增个体暴露面（离线玩家等级变可查）；由「组默认关 + strict 仅在线」两道收敛（见 §4.1）。
...
4. **自助 `bind` 无归属校验 + 可为他人开启隐藏（griefing）**：诚实登记，缓解=hidden_by 审计 + 本人自救（bind 己名 + me show）+ 管理员排除名单权威。详见 §5.5。
...
- **隐私三态**：strict（player **且** me 各自断言无时长字段、rank 砍时长榜且等级榜仅在线）、exclude 名单（同名多 key 全部命中、改名不旁路）、self-hide 过滤，各断言目标 key 不出现在 rank/player 且 bind 回 not_found。
...
4. 查询/格式化：`QueryService` 扩 rank（两榜 + 共享 day-bounds + `_load_excluded_keys` + strict/仅在线等级榜）、player（逐人 + not_found + `_apply_strict`）；`_load_excluded_keys`/`_apply_strict` 单点。
...
| P3 | 隐私/major | 等级榜含离线全体=新暴露面 | 采纳：登记暴露 + strict 仅在线（§4.1、§5.4） |
