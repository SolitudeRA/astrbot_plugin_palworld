...
**command_permissions 导入方（re-point 面，grep 核实）**：8 源文件（`config.py`·`container.py`·presentation 的 `admin_write_flow`/`commands`/`command_support`/`config_view`/`formatters`/`read_commands`）+ **17 测试文件**。
...
### 3.2 重指 25 导入方
`application.command_permissions` → `shared.command_permissions`：
- **8 源文件**：`config.py`（`from .application.command_permissions` → `from .shared.command_permissions`）、`container.py`（同）、6 presentation（`from ..application.command_permissions` → `from ..shared.command_permissions`）。
- **17 测试文件**：`from palworld_terminal.application.command_permissions import ...` → `from palworld_terminal.shared.command_permissions import ...`（清单见 §8）。
...
- **等价性（Part 1 move）**：靠全库 pytest 回归（command_permissions 的现有测试 `command_permissions_effective/endpoints/migrate_test` + config/container/gating/permissions 等 17 测试导入方，全走真实例，任何路径/行为漂移转红）+ import-linter 契约（结构等价的传递证据）。
...
- **17 测试导入方清单**（re-point，须逐一改 import 路径）：`commands_admin_write` · `commands_dispatch` · `commands_gating` · `commands_guild` · `commands_permissions` · `commands_player` · `commands_rank` · `command_permissions_effective` · `command_permissions_endpoints` · `command_permissions_migrate` · `config_command_permissions` · `container_features` · `frontend_pal_commands` · `gamedata_output_suppression` · `main_migration` · `rank_total` · `_perm`（+ 复核 grep 全仓 `application.command_permissions` 零残留）。
...
| 风险 | 缓解 |
|---|---|
| move 后漏改某导入方 → `ImportError`/`ModuleNotFoundError` | mypy `name-defined` + 全库 pytest（导入方炸）+ 全仓 grep `application.command_permissions` 零残留复核 |
...
- **改 25 导入方**：8 源（config/container + 6 presentation）+ 17 测试，`application.command_permissions` → `shared.command_permissions`。
...
1. **T1**：move `command_permissions` → shared（§3.1 改 import-depth）+ 重指 25 导入方（§3.2）+ 删原文件（§3.3）。验收：全库 pytest 1198（此刻 grep 守卫仍在、仍绿，move 不碰 application↛presentation/adapters）+ ruff + mypy + grep 零残留。**原子提交**（导入方须同提交切换否则中途 ImportError）。
