...
- **零行为变化**：现有全库测试保持全绿。基线 **1201 passed / 1 skipped**（main @ 80451e2）→ T3 删 2 grep 守卫后 **1199 passed / 1 skipped**。契约走 CI lint 步、非 pytest，不加计。
...
Expected: ruff `All checks passed!`；mypy `Success: no issues found in 64 source files`（move 非新增文件，净 0）；pytest `1201 passed, 1 skipped`（含 8 个 import main 的测试 + 18 个 import command_permissions 的测试全绿=重指完整的证据）。若某 test collection 失败=漏改某导入方，回 Step 2/4。
...
Expected: ruff clean；mypy Success 64；pytest `1201 passed, 1 skipped`（契约非 pytest，count 不变）；lint-imports `2 kept, 0 broken`。
...
- [ ] **Step 4: 全套验收（−2 守卫 → 1199）**

Run: `.venv/Scripts/ruff.exe check . && .venv/Scripts/python.exe -m mypy palworld_terminal/ && .venv/Scripts/python.exe -m pytest -q && .venv/Scripts/lint-imports.exe`
Expected: ruff clean；mypy Success 64；pytest **`1199 passed, 1 skipped`**（1201 − 2 删守卫）；lint-imports `2 kept, 0 broken`。
...
- `lint-imports`（0 broken）+ `ruff check .` + `mypy(64)` + `pytest`（1199 passed/1 skipped）+ CI 全绿。
...
- 计数一致：27 导入方（9 源 + 18 测试）、1201→1199、mypy 64——与 spec 一致。
