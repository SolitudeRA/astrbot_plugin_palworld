...
| 功能组 | 默认 | 命令 | 说明 |
|------|------|------|------|
| `core` | 恒开(不可关) | `world status/rules` `online` `whoami` `help` `confirm` `link`(裸) `server`(裸) | 基础命令,始终可用 |
...
- **审计留存与 PII**:`admin_audit` 表含**明文** `admin_id`(授权管理员标识)、`target_name`(目标角色名)与时间,属受控 PII;目标 userid 仅以**哈希**(与观测侧同一 world_id 命名空间)落库,不存明文。默认 180 天有界清理,可按需调整。
