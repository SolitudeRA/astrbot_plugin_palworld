...
| `/pal world overview` | — | `guilds_bases` | 世界概览(公会/据点世界快照;归 `guilds_bases` 组,默认开) |
...
| `guilds_bases` | **开** | `world overview` `guild list` `guild info` `guild bases` `guild base` `dex` | ✅ 可用(`/game-data` 端点随之轮询) | ❌ 关闭时回「未开启」、help 隐藏 |
